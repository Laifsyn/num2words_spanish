...
//         vec!["femenino", /* "f", "feminine", */
// "plural"].into_iter().map(String::from).collect();     let prefs_for_decimal_char: Vec<String> =
// vec!["coma"].into_iter().map(String::from).collect();
...
//     #[rustfmt::skip]
//     assert_eq!(word, "ochocientos veintiún millones cuatrocientos cuarenta y dos mil quinientos
// veinticuatro coma seis nueve negativo");     let word = driver.to_ordinal(BigFloat::from(-484));
